...
class CoupletProcessor(DataProcessor):
  def get_train_examples(self, data_dir):
    input_path = os.path.join(data_dir, "train", "in.txt")
    output_path = os.path.join(data_dir, "train", "out.txt")
    return self._create_examples( self._read_txt(input_path), self._read_txt(output_path), "train")
  def get_dev_examples(self, data_dir):
    input_path = os.path.join(data_dir, "test", "in.txt")
    output_path = os.path.join(data_dir, "test", "out.txt")
    return self._create_examples(self._read_txt(input_path), self._read_txt(output_path), "test")
  def get_test_examples(self, data_dir):
    return self.get_dev_examples(data_dir)
  def get_labels(self):
    return ["0", "1"]
  def _create_examples(self, intput_lines, output_lines, set_type):

    examples = []
    for (i, line) in enumerate(intput_lines):
      guid = "%s-%s" % (set_type, i)
      text_a = self.process_text(intput_lines[i].replace(" ", "").strip())
      text_b = self.process_text(output_lines[i].replace(" ", "").strip())
      label = "1"
      #if i % 1000 == 0:
      #  print (i)
      #print ("-=-=-=-=-=-=-=-==-=-=-=-=-=-")
      #print (text_a)
      #print (text_b)
      #print (self._generate_same_case(text_a))
      #print (self._generate_short_case(text_b))
      #print (self._generate_long_case(text_b))
      #print (self._generate_reorder_case(text_b))
      #print (self._generate_random_case(i, output_lines))
      #print ("-=-=-=-=-=-=-=-==-=-=-=-=-=-")

      examples.append(
          InputExample(guid=guid+"-0", text_a=text_a, text_b=text_b, label="1"))

      examples.append(
          InputExample(guid=guid+"-1", text_a=text_a, text_b=self._generate_same_case(text_a), label="0"))

      examples.append(
          InputExample(guid=guid+"-2", text_a=text_a, text_b=self._generate_short_case(text_b), label="0"))

      examples.append(
          InputExample(guid=guid+"-3", text_a=text_a, text_b=self._generate_long_case(text_b), label="0"))

      examples.append(
          InputExample(guid=guid+"-4", text_a=text_a, text_b=self._generate_reorder_case(text_b), label="0"))

      examples.append(
          InputExample(guid=guid+"-5", text_a=text_a, text_b=self._generate_short_case(text_b), label="0"))

      examples.append(
          InputExample(guid=guid+"-6", text_a=text_a, text_b=self._generate_long_case(text_b), label="0"))

      examples.append(
          InputExample(guid=guid+"-7", text_a=text_a, text_b=self._generate_reorder_case(text_b), label="0"))

      examples.append(
          InputExample(guid=guid+"-8", text_a=text_a, text_b=self._generate_random_case(i, output_lines), label="0"))

      examples.append(
          InputExample(guid=guid+"-9", text_a=text_a, text_b=self._generate_random_case(i, output_lines), label="0"))

      examples.append(
          InputExample(guid=guid+"-10", text_a=text_a, text_b=self._generate_random_case(i, output_lines), label="0"))

    print (len(examples))
    return examples

  def _generate_short_case(self, string):
    length = len(string)
    n = random.randint(0,length-1)
    str = string[:n] + string[n+1:]
    return str

  def _generate_same_case(self, string):
    return string

  def _generate_long_case(self, string):
    length = len(string)
    n = random.randint(0,length-1)
    str = string + string[n]
    return str

  def _generate_reorder_case(self, string):
    length = len(string)
    if length == 1:
      return ""
    n = 0
    while n == 0:
      n = random.randint(0,length-1)
    str = string[n:] + string[0:n]
    return str

  def _generate_random_case(self, index, strings):
    length = len(strings)
    n = index
    while n == index:
      n = random.randint(0,length-1)
    return strings[n].replace(" ", "").strip()
